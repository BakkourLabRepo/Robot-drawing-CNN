...
def main():
    ...
    parser.add_argument('--core', type=str, choices=["CPU", "GPU"], required=True, help='Modality: CPU vs GPU')
    ...
    # ModelPerformance directory
    ModelPerformance_DIR = "ModelPerformance"
    os.makedirs(ModelPerformance_DIR, exist_ok=True)
    ...
    with strategy.scope():
        ...
        if last_checkpoint:
            ...
            model = compile_model(model_name = model_name, 
    ...
    with open(f"{ModelPerformance_DIR}/{model_name}-Model Performance.txt", "w") as f:
        with contextlib.redirect_stdout(f):
            ...
            _, _, true_labels = data_split(label_df, SPLIT_RATIO, RANDOM_STATE)
            print("---------------------------------------------------------------", 
                file=f)

            print("Evaluate the prediction", file=f)
            classifi_report, confusion_matrix, roc_auc = \
                test_predict_eval(binary_prediction, true_labels, LABELS)
